/*
   ...
   When the right code entered it you will be ask to choose the port to change
   toggle when * pressed last entered digit will be deleted. when # pressed the
   code will be cleared to enter new one.
...
byte rowPins[ROWS] = {
    7, 6, 5,
    4};  // Connect keypad ROW0, ROW1, ROW2 and ROW3 to these Arduino pins.
byte colPins[COLS] = {
    10, 9, 8};  // Connect keypad COL0, COL1 and COL2 to these Arduino pins.
...
LiquidCrystal_I2C lcd(LCD_ADDRESS, LCD_COLUMNS,
                      LCD_ROWS);  // run ic2_scanner sketch and get the IC2
...
void loop() {
    ...
    if (code.length() ==
        KEY_SIZE) {  // if the array index is equal to the number of expected
...
void undoLastKey() {
    ...
    code = index == 0
               ? ""
               : code.substring(0, --index);  // remove one char from string
